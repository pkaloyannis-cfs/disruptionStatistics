...
def fasten_TCV():
    file_name = [
        "Table_TCV_Disruptions_RE.csv",
        "Table_TCV_Disruptions_below50kA.csv",
        "Table_TCV_Disruptions_VD.csv",
        "Table_TCV_Disruptions_RD.csv",
        "Table_TCV_Disruptions_VDE.csv",
        "Table_TCV_Disruptions_0.csv",
    ...
    if exp == "VD":
        ...
        all_files = sorted(
            (
                ...
                "Lite_Table_TCV_Disruptions_0.csv",
    ...
    if name == "_TCV":
        ...
        if exp == "VD":
            all_exp_file = machine_param.loc[
                machine_param["filename"] == "Lite_Table_TCV_Disruptions_VD.csv"
            ]
        if exp == "VDE":
            all_exp_file = machine_param.loc[
                machine_param["filename"] == "Lite_Table_TCV_Disruptions_VDE.csv"
            ]
...
def plot_disruptivity_1D(X, X_range, width, jj, denom_idx, deltat):
    ...
    plt.bar(
        bins[0:-1], vps, color="firebrick", width=width, align="center", zorder=2, edgecolor=None
    )
    plt.errorbar(
        bins[0:-1],
        vps,
...
def plt_1D(machine_param, X_range, jj_50, denom_idx, deltat, name, exp, param_name):
    x = machine_param
    plot_disruptivity_1D(x, X_range, (X_range[1] - X_range[0]) / 25, jj_50, denom_idx, deltat)
...
def plt_2D(
    ...
    plot_disruptiviy_2D(x, y, X_range, Y_range, jj_50, denom_idx, deltat, param_name1, param_name2)
...
if work_type == "1":
    ...
    jj_50, denom_idx = indices_operation(
        all_data,
        shots_list,
        indices_no_disrupt,
        indices_disrupt,
...
if __name__ == "__main__":
    ...
    if flag == "1D" or flag == "BOTH":
        ...
        plt_1D(all_data.KAPPA, KAPPA_range, jj_50, denom_idx, deltat, name, exp, "elongation")
        plt_1D(all_data.GAP_in, GAP_in_range, jj_50, denom_idx, deltat, name, exp, "GAP_in")
        plt_1D(all_data.GAP_out, GAP_out_range, jj_50, denom_idx, deltat, name, exp, "GAP_out")
