...
This file contains computation routines for computing disruptivity maps.
This includes methods for both the sampling and maximum likelihood approaches.

Note: Only sampling disruptivity functions are currently tested.
...
# TODO: Document the returns of some function
# TODO: Convert ARGS into a class that can be passed
# TODO(cont): around for returns.
...
def compute_disruptivity_sampling(
    ...
    Args:
        ...
        ret_dt_array (bool, optional): Return the dt_array used in division.
    """
    ...
    # Zero the error where we have masked the data.
    error[n_no_disrupt] = 0
    error[n_all_disrupt] = 0
    error[n_no_data] = 0

    if ret_dt_array:
        return disruptivity, error, num_dd.bin_edges, entry_dict, dt_array
    ...
    for i in range(denom_dd.binnumber.shape[-1]):
        ...
        if (
            ...
            if shotlist is not None:
                shotlist_bool = np.isin(shot, shotlist)
                is_disrupt = shotlist_bool
            else:
                is_disrupt = True
...
def p_data(d, dt, is_disrupt: bool):
    ...
    # Simple switch for if the data is disruptive or not disruptive.
    if is_disrupt:
...
def find_disruptivity(
    ...
    """Computes the likelihood disruptivity for a given pixel.
...
def compute_disruptivity_likelihood(
    ...
    entry_list, max_counter = compute_continuous_time_bin(
        dataframe, hist, indices, tau=tau, window=window, shotlist=shotlist
    )
...
def get_indices_disruptivity(
    ...
    """Adds the indices for disruptivity to the index dictionary.
    ...
    # Find Viable Times for Disruptivity and Denominator.
...
# --- Tau Disruptivity Trajectories ---
def compute_tau_disruptivity(
    tokamak_config,
    dataframe,
    index_dict,
    entry_dict,
    ...
    """Computes a 1D trajectory of disruptiviy for different tau times.
    ...
    Args:
        ...
        dataframe (pd.core.frame.DataFrame): The Tokamak Dataframe.
        index_dict (dict): The Index Dictionary.
        entry_dict ([type]): [description]
        tau_list ([type]): [description]
        window_list ([type]): [description]
        shotlist ([type], optional): [description]. Defaults to None.
        nbins (int, optional): [description]. Defaults to 35.
        type (str, optional): [description]. Defaults to 'sampling'.
    """

    # Safety asserts for type
    # TODO: Add ML for this.
    assert type in [
        "sampling"
    ], f"Invalid type {type} in tau disruptivity calculation."
    ...
    for i, (tau, window) in enumerate(zip(tau_list, window_list)):
        ...
        args = dis.compute_disruptivity_sampling(
            dataframe,
            entry_dict,
            num_indices,
            denom_indices,
        ...
        # On the first cycle save the dt array and bins
        if i == 0:
            bins = args[2]
            dt_array = args[-1]
    ...
    return results, errors, bins, entry_dict, dt_array
