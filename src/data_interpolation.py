...
def plot_interp(df_interpolated):
    if name == "_CMod":
        plt.plot(
            df_interpolated.Time.loc[df_interpolated.Shot == 1120105029],
...
def plot_negative(all_data):
    if name == "_CMod":
        plt.plot(
            all_data.Time.loc[all_data.Shot == 1120105029],
...
def interp_data(all_data, name, exp, param_name):
    ...
    if len(shot_u) == 0:
        ...
        for i in shot_u:
            sep_shot_param = (
                all_data_new[param_name]
                .loc[all_data_new.Shot == i]
...
if name == "_TCV":
    ...
    all_data.loc[all_data.filename == "Lite_Table_TCV_Disruptions_0.csv"].to_csv(
        "Lite_Table_TCV_Disruptions_0_new.csv", index=False
    )
    all_data.loc[all_data.filename == "Lite_Table_TCV_NonDisruptions.csv"].to_csv(
        "Lite_Table_TCV_NonDisruptions_new.csv", index=False
    )
    if exp == "VD":
        all_data.loc[all_data.filename == "Lite_Table_TCV_Disruptions_VD.csv"].to_csv(
            "Lite_Table_TCV_Disruptions_VD_new.csv", index=False
        )
    if exp == "VDE":
        all_data.loc[all_data.filename == "Lite_Table_TCV_Disruptions_VDE.csv"].to_csv(
