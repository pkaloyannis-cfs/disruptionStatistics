...
def read_data_TCV(exp):
    ...
    if exp == "VD":
        ...
        all_files = sorted(
            (
                "Lite_Table_TCV_Disruptions_VD_new.csv",
                "Lite_Table_TCV_Disruptions_0_new.csv",
                "Lite_Table_TCV_NonDisruptions_new.csv",
            )
        )
        tcv_all = pd.concat(
            (pd.read_csv(file).assign(filename=file) for file in all_files), ignore_index=True
        )
    ...
    if exp == "VDE":
        ...
        all_files = sorted(
            (
                "Lite_Table_TCV_Disruptions_VDE_new.csv",
                "Lite_Table_TCV_Disruptions_0_new.csv",
                "Lite_Table_TCV_NonDisruptions_new.csv",
            )
        )
...
def define_indices_CMod(cmod):
    ...
    indices_flattop = list(cmod[abs(cmod.dipprog_dt) <= 1.0e3].index)
    indices_disrupt_time = list(cmod[cmod.Time2Disr <= 3e-3].index)
    return indices_no_disrupt, indices_disrupt, indices_flattop, indices_disrupt_time


def indices_operation(
    machine_param,
    shots_list,
    indices_no_disrupt,
    indices_disrupt,
    indices_flattop,
    indices_disrupt_time,
    name,
    exp,
):
    ...
    indices_disrupt_in_flattop = list(
        machine_param.Shot[
            np.isin(machine_param.Shot, machine_param.Shot[indices_disrupt_time_in_flattop])
        ].index
    )
    ...
    if name == "_TCV":
        ...
        other_disruptions = list(
            machine_param.loc[
                machine_param["filename"]
                == "C://Users//nadya//Documents//Python"
                " Scripts//causal_inf_cross//Lite_Table_TCV_Disruptions_0_new.csv"
            ].index
        )
        # all VDE/VD data reading
        if exp == "VD":
            all_exp_file = machine_param.loc[
                machine_param["filename"]
                == "C://Users//nadya//Documents//Python"
                " Scripts//causal_inf_cross//Lite_Table_TCV_Disruptions_VD_new.csv"
            ]
        if exp == "VDE":
            all_exp_file = machine_param.loc[
                machine_param["filename"]
                == "C://Users//nadya//Documents//Python"
                " Scripts//causal_inf_cross//Lite_Table_TCV_Disruptions_VDE_new.csv"
            ]
        ...
        jj_before_40 = list(
            np.intersect1d(jj, list(all_exp_file[all_exp_file.Time2Disr <= 0.04].index))
        )
        ...
        jj_50 = list(
            np.intersect1d(
                jj,
                all_exp_file.loc[
                    (all_exp_file["Time2Disr"] >= 0.04) & (all_exp_file["Time2Disr"] <= 0.06)
                ].index,
            )
        )
    ...
    if name == "_CMod":
        ...
        # include data for VDEs flattop where time_until_disrupt<=40ms
        jj_before_40 = list(
            np.intersect1d(jj, list(machine_param[machine_param.Time2Disr <= 0.04].index))
        )
        # isolate for VDE shots behavior around 50ms before disruption
        jj_50 = list(
            np.intersect1d(
                jj,
                machine_param.loc[
                    (machine_param["Time2Disr"] >= 0.04) & (machine_param["Time2Disr"] <= 0.06)
                ].index,
            )
        )
...
if work_type == "2":
    ...
    indices_no_disrupt, indices_disrupt, indices_flattop, indices_disrupt_time = (
        define_indices_CMod(all_data)
    )
    jj_50, ii = indices_operation(
        all_data,
        shots_list,
...
def step_1(params_list, param_to_expl, normalized_all_data, ii, jj_50, D, flag):
    X = pd.concat(
        [
            normalized_all_data.loc[normalized_all_data.index.isin(ii)],
            normalized_all_data.loc[normalized_all_data.index.isin(jj_50)],
        ]
    ).sort_index()
    X = X.dropna()
    Y = X[param_to_expl]
...
D = pd.concat(
    [pd.Series(data=[0] * len(ii), index=ii), pd.Series(data=[1] * len(jj_50), index=jj_50)]
).sort_index()  # if index is in jj_50 - than 1, if not - 0 - (now just for not disruptive)
params = {
    "DELTA_BOTTOM",
    "DELTA",
    "DELTA_TOP",
...
for i in param:
    ...
    if "X_1" in data_cm:
        ...
        data_cm["X_1"] = [step_1(params, param[n], normalized_all_data[n], ii, jj_50, D, flag=0)[2]]
    if "Y_1" in data_cm:
        data_cm["Y_1"].append(
            step_1(params, param[n], normalized_all_data[n], ii, jj_50, D, flag=0)[3]
        )
    else:
        data_cm["Y_1"] = [step_1(params, param[n], normalized_all_data[n], ii, jj_50, D, flag=0)[3]]
    ...
    if "D_1" in data_cm:
        ...
        data_cm["D_1"] = [step_1(params, param[n], normalized_all_data[n], ii, jj_50, D, flag=0)[1]]
...
for i in range(len(data_cm["cm_1"])):
    ...
    for j in range(len(data_cm["X_1"][i].columns)):
        print(
            "\t", data_cm["X_1"][i].columns[j], " - ", data_cm["cm_1"][i].propensity["coef"][j + 1]
        )
...
for i in param:
    ...
    [
        explore_last_strata(
            i, data_cm["stratas"][n], data_cm["X_1"][n], data_cm["Y_1"][n], data_cm["D_1"][n], flag
        )
    ]
...
for i in range(len(data_cm["cm_1"])):
    if "X_12_orig" in data_cm_final:
        data_cm_final["X_12_orig"].append(all_data[params].iloc[data_cm_final["X_12"][i].index])
    else:
        data_cm_final["X_12_orig"] = [all_data[params].iloc[data_cm_final["X_12"][i].index]]
...
for i in range(len(data_cm["cm_1"])):
    if "Y_12_orig" in data_cm_final:
        data_cm_final["Y_12_orig"].append(all_data[param[i]].iloc[data_cm_final["X_12"][i].index])
    else:
        data_cm_final["Y_12_orig"] = [all_data[param[i]].iloc[data_cm_final["X_12"][i].index]]
...
for i in range(len(param)):
    ...
    if "cm_orig" in data_cm_final:
        data_cm_final["cm_orig"].append(
            CausalModel(
                data_cm_final["Y_12_orig"][i].values,
                data_cm_final["D_12_orig"][i].values,
                data_cm_final["X_12_orig"][i][params].drop({param[i]}, axis=1).values,
            )
        )
    else:
        data_cm_final["cm_orig"] = [
            CausalModel(
                data_cm_final["Y_12_orig"][i].values,
                data_cm_final["D_12_orig"][i].values,
                data_cm_final["X_12_orig"][i][params].drop({param[i]}, axis=1).values,
...
for i in range(len(param)):
    ...
    treated_ind = list(data_cm_final["D_12_orig"][i].loc[data_cm_final["D_12_orig"][i] == 1].index)
    control_ind = list(data_cm_final["D_12_orig"][i].loc[data_cm_final["D_12_orig"][i] == 0].index)

    plt.scatter(
        all_data[param[i]].iloc[control_ind],
        data_cm["X_1"][i]["propensity_score"][control_ind],
        marker="o",
        c="b",
        alpha=0.5,
        label="Control",
    ...
    plt.scatter(
        all_data[param[i]].iloc[treated_ind],
        data_cm["X_1"][i]["propensity_score"][treated_ind],
        marker="o",
        c="r",
        alpha=0.7,
        label="Treatment",
...
if name == "_CMod":
    df_CMod = pd.DataFrame(
        data={"ATE": ATE_CMod, "ATT": ATT_CMod, "ATC": ATC_CMod},
        index=ind_name,
    )
    ...
    for i in range(len(df_CMod.index)):
        df_CMod.iloc[i].plot.bar(
            ax=axes[i],
            title=f"Treatment effects CMod - {ind_name[i]}",
...
if name == "_TCV":
    if exp == "VDE":
        df_TCV_VDE = pd.DataFrame(
            data={"ATE": ATE_TCV_VDE, "ATT": ATT_TCV_VDE, "ATC": ATC_TCV_VDE},
            index=ind_name,
        )
        fig, axes = plt.subplots(figsize=(60, 6), nrows=1, ncols=10)
        # ax2=plt.subplot(2,2,2)
    ...
    if exp == "VD":
        df_TCV_VD = pd.DataFrame(
            data={"ATE": ATE_TCV_VD, "ATT": ATT_TCV_VD, "ATC": ATC_TCV_VD},
            index=ind_name,
        )
        fig, axes = plt.subplots(figsize=(60, 6), nrows=1, ncols=10)
        # ax2=plt.subplot(2,2,2)
